...
impl Default for RangeArray2Stack {
    fn default() -> RangeArray2Stack {
        RangeArray2Stack::zeros(((0, 0), (0, 0)))
    }
}

impl RangeArray2Stack {
    // iterate over indexes and values
    pub fn iter(&self) -> impl Iterator<Item = (i64, i64, &f64)> + '_ {
        self.array.iter().enumerate().map(|(idx, v)| {
            (
    ...
    pub fn new(values: &Vec<((i64, i64), f64)>, cstt: f64) -> RangeArray2Stack {
        if values.is_empty() {
            return RangeArray2Stack {
                min: (0, 0),
                max: (0, 0),
                nb0: 0,
                array: [0.; 10000],
        ...
        if nb0 * (max.1 - min.1 + 1) > 10000 {
            panic!("Too much data in RangeArray2Stack")
        }

        let mut array = [cstt; 10000];
        for ((i0, i1), value) in values {
            array[(i0 - min.0) as usize + ((i1 - min.1) * nb0) as usize] += value;
        }
        RangeArray2Stack {
    ...
    pub fn get_mut(&mut self, idx: (i64, i64)) -> &mut f64 {
        ...
        unsafe {
            println!(
                ...
                self.min,
                self.max,
                idx.0 - self.min.0 + (idx.1 - self.min.1) * self.nb0
            );
            self.array
    ...
    pub fn zeros(range: ((i64, i64), (i64, i64))) -> RangeArray2Stack {
        RangeArray2Stack {
            min: range.0,
            max: range.1,
            nb0: range.1 .0 - range.0 .0,
    ...
    pub fn constant(range: ((i64, i64), (i64, i64)), cstt: f64) -> RangeArray2Stack {
        RangeArray2Stack {
            min: range.0,
            max: range.1,
            nb0: range.1 .0 - range.0 .0,
